...
/* Remove a record from list "list" and copy attributes into transfer.
   ...
   option = FIRST remove first record in the list
    ...
    puts("decrement list size");
    list_size[list]--;

    /* If the option value is improper, stop the simulation. */
    puts("test for improper value");
    ...
    if(list_size[list] == 0) {
        puts("only one value found in list");
        ...
        puts("more than one value found in list");
...
/* Remove next event from event list, placing its attributes in transfer.
   ...
   Set next_event_type to this event type, transfer[2]. */
    ...
    puts("Remove next event from event list");
    list_remove(FIRST, LIST_EVENT);

    /* Check for a time reversal. */
    puts("check for time reversal");
    ...
    puts("advance sim clock");
    sim_time        = transfer[EVENT_TIME];
    puts("set next event type");
                                                        ...
                                                        generation function. */
{
    ...
    puts("use lcgrand to get rand int");
    u = lcgrand(stream);

    puts("return the index of prob_distr determined by lcgrand");
